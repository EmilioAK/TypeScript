...
export function setStackTraceLimit() {
  if ((Error as any).stackTraceLimit < 100) {
    // Also tests that we won't set the property if it doesn't exist.
...
function setCustomPollingValues(system: System) {
  ...
  const pollingIntervalChanged = setCustomLevels(
  ...
  function getCustomPollingBasedLevels(
    baseVariable: string,
    ...
    return (
      ...
      createPollingIntervalBasedLevels(
        customLevels ? { ...defaultLevels, ...customLevels } : defaultLevels,
      )
    );
  }
}
...
function pollWatchedFileQueue<T extends WatchedFileWithIsClosed>(
  ...
  for (
    let canVisit = queue.length;
    chunkSize && canVisit;
    nextPollIndex(), canVisit--
  ) {
    ...
    const fileChanged = onWatchedFileStat(
      watchedFile,
      getModifiedTime(host, watchedFile.fileName),
    );
...
function createDynamicPriorityPollingWatchFile(host: {
  ...
  function pollQueue(
    queue: (WatchedFileWithUnchangedPolls | undefined)[],
    pollingInterval: PollingInterval,
    pollIndex: number,
    chunkSize: number,
  ) {
    ...
    function onWatchFileStat(
      watchedFile: WatchedFileWithUnchangedPolls,
      pollIndex: number,
      fileChanged: boolean,
    ) {
      if (fileChanged) {
        ...
        watchedFile.unchangedPolls !== unchangedPollThresholds[pollingInterval]
      ) {
  ...
  function addToPollingIntervalQueue(
    file: WatchedFileWithUnchangedPolls,
    pollingInterval: PollingInterval,
  ) {
...
function createUseFsEventsOnParentDirectoryWatchFile(
  ...
  const fileTimestamps = fsWatchWithTimestamp
    ? new Map<string, Date>()
    : undefined;
  ...
  function nonPollingWatchFile(
    ...
    if (
      fileWatcherCallbacks.add(filePath, callback).length === 1 &&
      fileTimestamps
    ) {
      fileTimestamps.set(
        filePath,
        getModifiedTime(fileName) || missingFileModifiedTime,
      );
    }
...
function createFixedChunkSizePollingWatchFile(host: {
  ...
  function pollQueue() {
    ...
    pollIndex = pollWatchedFileQueue(
      host,
      watchedFiles,
      pollIndex,
      pollingChunkSize[PollingInterval.Low],
    );
    scheduleNextPoll();
  }
...
function createSingleWatcherPerName<
  T extends FileWatcherCallback | FsWatchCallback,
>(
  ...
  if (existing) {
    ...
    cache.set(path, {
      watcher: createWatcher(
        ...
        ((param1: any, param2: never, param3: any) =>
          cache
            .get(path)
            ?.callbacks.slice()
            .forEach((cb) => cb(param1, param2, param3))) as T,
      ),
      callbacks: [callback],
    });
  }
  ...
  return {
    close: () => {
      ...
      if (!watcher) return;
      if (
        !orderedRemoveItem(watcher.callbacks, callback) ||
        watcher.callbacks.length
      )
        return;
      cache.delete(path);
...
function createDirectoryWatcherSupportingRecursive({
  ...
  const callbackCache = createMultiMap<
    Path,
    { dirName: string; callback: DirectoryWatcherCallback }
  >();
        ...
        watcher: watchDirectory(
          ...
          (fileName) => {
            ...
            if (options?.synchronousWatchDirectory) {
              ...
              if (!cache.get(dirPath)?.targetWatcher)
  ...
  function invokeCallbacks(
    dirName: string,
    dirPath: Path,
    fileNameOrInvokeMap: string | InvokeMap,
    fileNames?: string[],
  ) {
    ...
    callbackCache.forEach((callbacks, rootDirName) => {
      ...
      if (
        rootDirName === dirPath ||
        (startsWith(dirPath, rootDirName) &&
          dirPath[rootDirName.length] === directorySeparator)
      ) {
        if (invokeMap) {
          if (fileNames) {
    ...
    cache.get(dirPath)?.links?.forEach((link) => {
      const toPathInLink = (fileName: string) =>
        combinePaths(
          link,
          getRelativePathFromDirectory(dirName, fileName, toCanonicalFilePath),
        );
      if (invokeMap) {
        invokeCallbacks(
          link,
          toCanonicalFilePath(link),
          invokeMap,
          fileNames?.map(toPathInLink),
        );
      } else {
        invokeCallbacks(
          link,
          toCanonicalFilePath(link),
          toPathInLink(fileName!),
        );
      }
    });
  }
  ...
  function nonSyncUpdateChildWatches(
    ...
    if (
      parentWatcher &&
      fileSystemEntryExists(dirName, FileSystemEntryKind.Directory)
    ) {
  ...
  function onTimerToUpdateChildWatches() {
    timerToUpdateChildWatches = undefined;
    sysLog(
      `sysLog:: onTimerToUpdateChildWatches:: ${cacheToUpdateChildWatches.size}`,
    );
    ...
    while (!timerToUpdateChildWatches && cacheToUpdateChildWatches.size) {
      ...
      const {
        value: [dirPath, { dirName, options, fileNames }],
      } = result;
      ...
      if (!cache.get(dirPath)?.targetWatcher)
        invokeCallbacks(
          ...
          dirPath,
          invokeMap,
          hasChanges ? undefined : fileNames,
        );
    }

    sysLog(
      `sysLog:: invokingWatchers:: Elapsed:: ${timestamp() - start}ms:: ${cacheToUpdateChildWatches.size}`,
    );
    callbackCache.forEach((callbacks, rootDirName) => {
      const existing = invokeMap.get(rootDirName);
      if (existing) {
  ...
  function updateChildWatches(
    ...
    if (filePathComparer(target, parentDir) === Comparison.EqualTo) {
      ...
      hasChanges = enumerateInsertsAndDeletes<string, ChildDirectoryWatcher>(
        fileSystemEntryExists(parentDir, FileSystemEntryKind.Directory)
          ? mapDefined(
              getAccessibleSortedChildDirectories(parentDir),
              (child) => {
                const childFullName = getNormalizedAbsolutePath(
                  child,
                  parentDir,
                );
                ...
                return !isIgnoredPath(childFullName, options) &&
                  filePathComparer(
                    childFullName,
                    normalizePath(realpath(childFullName)),
                  ) === Comparison.EqualTo
                  ? childFullName
                  : undefined;
              },
      ...
      parentWatcher.targetWatcher &&
      filePathComparer(target, parentWatcher.targetWatcher.dirName) ===
        Comparison.EqualTo
    ) {
      hasChanges = false;
  ...
  function isIgnoredPath(path: string, options: WatchOptions | undefined) {
    return (
      some(ignoredPaths, (searchPath) => isInPath(path, searchPath)) ||
...
export type FsWatchCallback = (
  ...
  modifiedTime?: Date,
) => void; // eslint-disable-line no-restricted-syntax
/** @internal */
export type FsWatch = (
...
function createFileWatcherCallback(
  callback: FsWatchCallback,
): FileWatcherCallback {
  return (_fileName, eventKind, modifiedTime) =>
    callback(
  ...
  return (eventName, relativeFileName) => {
    ...
    if (eventName === "rename") {
      ...
      const fileName = !relativeFileName
        ? directoryName
        : normalizePath(combinePaths(directoryName, relativeFileName));
      if (
        !relativeFileName ||
        !isIgnoredByWatchOptions(
          fileName,
...
/** @internal */
export type FileSystemEntryExists = (
  fileorDirectrory: string,
  entryKind: FileSystemEntryKind,
) => boolean;
...
export function createSystemWatchFunctions({
  ...
  function ensureDynamicPollingWatchFile() {
    return (dynamicPollingWatchFile ||= createDynamicPriorityPollingWatchFile({
      getModifiedTime,
      setTimeout,
    }));
  }
  ...
  function ensureFixedChunkSizePollingWatchFile() {
    return (fixedChunkSizePollingWatchFile ||=
      createFixedChunkSizePollingWatchFile({ getModifiedTime, setTimeout }));
  }

  function updateOptionsForWatchFile(
    options: WatchOptions | undefined,
    useNonPollingWatchers?: boolean,
  ): WatchOptions {
    ...
    switch (tscWatchFile) {
      ...
      case "UseFsEventsWithFallbackDynamicPolling":
        ...
        return generateWatchFileOptions(
          WatchFileKind.UseFsEvents,
          PollingWatchKind.DynamicPriority,
          options,
        );
  ...
  function generateWatchFileOptions(
    ...
    return {
      watchFile,
      fallbackPolling:
        defaultFallbackPolling === undefined
          ? fallbackPolling
  ...
  function watchDirectory(
    ...
    return hostRecursiveDirectoryWatcher(
      directoryName,
      callback,
      recursive,
      options,
  ...
  function nonRecursiveWatchDirectory(
    ...
    const watchDirectoryKind = Debug.checkDefined(
  ...
  function fsWatchHandlingExistenceOnHost(
    ...
    if (inodeWatching) {
      lastDirectoryPartWithDirectorySeparator = fileOrDirectory.substring(
        fileOrDirectory.lastIndexOf(directorySeparator),
      );
      lastDirectoryPart = lastDirectoryPartWithDirectorySeparator.slice(
        directorySeparator.length,
      );
    }
    /** Watcher for the file system entry depending on whether it is missing or present */
    let watcher: FileWatcher | undefined = !fileSystemEntryExists(
      fileOrDirectory,
      entryKind,
    )
    ...
    function watchPresentFileSystemEntry(): FileWatcher {
      ...
      try {
        const presentWatcher = (
          entryKind === FileSystemEntryKind.Directory || !fsWatchWithTimestamp
            ? fsWatchWorker
    ...
    function callbackChangingToMissingFileSystemEntry(
      event: "rename" | "change",
      relativeName: string | undefined | null,
    ) {
      ...
      if (
        ...
        const modifiedTime =
          getModifiedTime(fileOrDirectory) || missingFileModifiedTime;
  ...
  function fsWatchWorkerHandlingTimestamp(
    ...
    let modifiedTime =
      getModifiedTime(fileOrDirectory) || missingFileModifiedTime;
    return fsWatchWorker(
      fileOrDirectory,
      recursive,
      (eventName, relativeFileName, currentModifiedTime) => {
        if (eventName === "change") {
          currentModifiedTime ||=
            getModifiedTime(fileOrDirectory) || missingFileModifiedTime;
...
export interface System {
  ...
  /** @internal */ getAccessibleFileSystemEntries?(
    path: string,
  ): FileSystemEntries;
...
export let sys: System = (() => {
  ...
  function getNodeSystem(): System {
    const nativePattern =
      /^native |^\([^)]+\)$|^(internal[\\/]|[a-zA-Z0-9_\s]+(\.js)?$)/;
    ...
    const executingFilePath = __filename.endsWith("sys.js")
      ? _path.join(_path.dirname(__dirname), "__fake__.js")
      : __filename;
    ...
    const nodeSystem: System = {
      ...
      cpuProfilingEnabled: () =>
        !!activeSession ||
        contains(process.execArgv, "--cpu-prof") ||
        contains(process.execArgv, "--prof"),
      realpath,
      debugMode:
        !!process.env.NODE_INSPECTOR_IPC ||
        !!process.env.VSCODE_INSPECTOR_OPTIONS ||
        some(process.execArgv, (arg) =>
      ...
      setBlocking: () => {
        const handle = (process.stdout as any)?._handle as {
      ...
      for (const node of profile.nodes) {
        if (node.callFrame.url) {
          ...
          if (containsPath(fileUrlRoot, url, useCaseSensitiveFileNames)) {
            ...
            node.callFrame.url = (
              remappedPaths.has(url)
                ? remappedPaths
                : remappedPaths.set(url, `external${externalFileCounter}.js`)
            ).get(url)!;
            externalFileCounter++;
          }
        }
      ...
      if (activeSession && activeSession !== "stopping") {
        ...
        activeSession.post("Profiler.stop", (err, { profile }) => {
          if (!err) {
            try {
              if (statSync(profilePath)?.isDirectory()) {
                profilePath = _path.join(
                  profilePath,
                  `${new Date().toISOString().replace(/:/g, "-")}+P${process.pid}.cpuprofile`,
                );
              }
            } catch {
            ...
            _fs.writeFileSync(
              profilePath,
              JSON.stringify(cleanupPaths(profile)),
            );
          }
    ...
    function fsWatchFileWorker(
      ...
      _fs.watchFile(
        fileName,
        { persistent: true, interval: pollingInterval },
        fileChanged,
      );
      ...
      function fileChanged(curr: import("fs").Stats, prev: import("fs").Stats) {
        ...
        const isPreviouslyDeleted =
          +prev.mtime === 0 || eventKind === FileWatcherEventKind.Deleted;
        if (+curr.mtime === 0) {
    ...
    function fsRealPathHandlingLongPath(path: string): string {
      return path.length < 260
        ? _fs.realpathSync.native(path)
        : _fs.realpathSync(path);
    }
